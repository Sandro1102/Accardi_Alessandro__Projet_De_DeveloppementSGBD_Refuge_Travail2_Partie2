...
namespace Accardi_Alessandro_Refuge_WPF.CoucheBaseDeDonnees
{
    internal class EntreeDAO : AccesDBBase<Entree>
    {
        ...
        protected override string AfficherListe => "ani_entree_afficher_liste_complete";
        protected override string? AfficherUnElement => null;              
        protected override string Insert => "ani_entree_insertion";
        ...
        protected override Entree ConvertirEnObjet(IDataReader reader)
        {
            ...
            int id = GetValueOrDefault<int>(reader, "entree_id");
        ...
        protected override void AssignerParametreSQL(NpgsqlCommand cmd, Entree objet)
        {
            ...
            cmd.Parameters.Add("p_date_entree", NpgsqlTypes.NpgsqlDbType.Date).Value = objet.Date;
        ...
        public async Task<List<Entree>> AfficherListeEntrees()
        {
            return await ExecuterFonctionRetourListeAsync(AfficherListe);
        }

        // -------------------------------------------------------
        // Retourne toutes les entrées liées à un animal
        // -------------------------------------------------------

        public async Task<List<Entree>> AfficherParAnimalAsync(string idAnimal)
        {
            List<Entree> retVal = new List<Entree>();

            using (var connexion = ConnexionDB.GetConnexion())
            {
                await connexion.OpenAsync();

                string sql = "SELECT * FROM ani_entree_afficher_par_animal(@p_identifiant_animal)";
        ...
        // -------------------------------------------------------
        // Retourne la date de la dernière entrée d'un animal
        ...
        public async Task<DateTime?> GetEntreeMaxAsync(string identifiant)
        {
            ...
            using (var connexion = ConnexionDB.GetConnexion())
            {
                ...
                string sql = "SELECT ani_entree_date_max(@p_identifiant_animal)";
                ...
                using (var cmd = new NpgsqlCommand(sql, connexion))
                {
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                            retVal = GetDateTimeSafe(reader, "ani_entree_date_max");
                    }
                }
            }
            return retVal;
